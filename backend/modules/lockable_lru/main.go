...
func (llru *LLRU[K, V]) addUnlockedWithoutLockingNorCheckingCapacity(key K, value V) (*Entry[K, V]) {
...
func (llru *LLRU[K, V]) Peek(key K) (value V, ok bool) {
	c.lock.RLock()
...
func (llru *LLRU[K, V]) ContainsOrAdd(key K, value V) (ok, evicted bool) {
	var k K
	var v V
	c.lock.Lock()
...
// recent-ness or deleting it for being stale, and if not, adds the value.
// Returns whether found and whether an eviction occurred.
func (llru *LLRU[K, V]) PeekOrAdd(key K, value V) (previous V, ok, evicted bool) {
	var k K
	var v V
	c.lock.Lock()
...
func (llru *LLRU[K, V]) Remove(key K) (present bool) {
	var k K
	var v V
	c.lock.Lock()
...
func (llru *LLRU[K, V]) Resize(size int) (evicted int) {
	var ks []K
	var vs []V
	c.lock.Lock()
...
func (llru *LLRU[K, V]) RemoveOldest() (key K, value V, ok bool) {
	var k K
	var v V
	c.lock.Lock()
...
func (llru *LLRU[K, V]) GetOldest() (key K, value V, ok bool) {
	c.lock.RLock()
